...
def run_pandoc(input_file, output_file): 
    ...
    """ 
    try:
        command = f"C:\\Users\\ohuwe\\AppData\\Local\\Pandoc\\pandoc.exe -f docx -t asciidoc --default-image-extension '.png' --extract-media=docx_conversion_media/ -o {output_file} {input_file}"
...
def main():
    ...
    media_folder = f"./docx_conversion_media/"

    run_pandoc(args.input, f"{args.to}_no_format.adoc")
    
    fix_asciidoc(f"{args.to}_no_format.adoc", f"{args.to}.adoc")
